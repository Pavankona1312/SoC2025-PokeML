...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        return -np.log(y_hat[y] + 1e-15)  # If labelling starts from zero.
    ...
    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        return y_hat, self._cross_entropy(y,y_hat)
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        der_inp_out = (self.out)*(1-self.out)
        return dz*der_inp_out
...
class Linear:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        self.dw = (dz.reshape((-1,1)))@(self.inp.T)
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
        ...
        self.hid = Linear(input_size=self.input_size,output_size=self.hidden_size,weight_init_fn=self.weight_init_fn,learning_rate=learning_rate) 
        self.out = Linear(input_size=self.hidden_size,output_size=self.output_size,weight_init_fn=self.weight_init_fn,learning_rate=learning_rate)
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        forw = self.hid.forward(x)
        forw = self.sigmoid.forward(forw)
        forw = self.out.forward(forw)
        yp = y
        return self.softmax.forward(z=forw.flatten(),y=yp)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        dz_o = self.softmax.backward(y,y_hat)
        da_h = self.out.backward(dz_o)
        dz_h = self.sigmoid.backward(da_h)
        di = self.hid.backward(dz_h)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for _ in range(n_epochs):
            x,y=shuffle(X_tr,y_tr,_)
            for i in range(len(y)):  
                x_i = x[i]
                y_i = y[i]
                y_hat, loss = self.forward(x_i, y_i) 
                self.backward(y_i, y_hat)
                self.step()
            train_list.append(self.compute_loss(X_tr,y_tr))
            test_list.append(self.compute_loss(X_test,y_test))
